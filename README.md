...
**Large `*features.npy` files** (SigLIP2 / ResNet / ViT / sentence-transformer caption vectors) are **not stored in this GitHub repo**. Download them from the Hugging Face dataset **[sijiey/Urban-Comfort-Potential](https://huggingface.co/datasets/sijiey/Urban-Comfort-Potential)** (open **Files and versions** on that page and download the `*features.npy` files you need), then place them under `data_post_embedding/` before running **code_0**.
